...
class NZDirInformer(CatInformer):
    ...
    def run(self):
        ...
        if self.config.szweightcol == "":
            ...
            raise KeyError(
                f"weight column {self.config.szweightcol} not found in input data!"
            )
        ...
        self.model = dict(
            distances=distances,
            szusecols=self.config.usecols,
            szweights=szweights,
            szvec=szvec,
            sz_mag_data=sz_mag_data,
        )
...
class NZDirSummarizer(CatEstimator):
    ...
    def run(self):
        ...
        for s, e, test_data in iterator:
            ...
            self._process_chunk(
                first, total_chunks, chunk_number, test_data, bootstrap_matrix
            )
            first = False
    ...
    def _process_chunk(
        self, first, number_of_chunks, chunk_number, test_data, bootstrap_matrix
    ):
        ...
        phot_mag_data[~np.isfinite(phot_mag_data)] = 40.0
        ...
        qp_dstn = qp.Ensemble(
            ...
            ancil=dict(normalization=np.array([normalization_factor])),
        )
        if first:
            self._single_handle = self.initialize_handle(
                "single_NZ", qp_dstn, number_of_chunks
            )
        self._do_chunk_output(
            qp_dstn, chunk_number, chunk_number + 1, self._single_handle
        )
        ...
        sample_ens = qp.Ensemble(
            ...
            ancil=dict(normalization=normalization_factor),
        )
    ...
    def join_histograms(self):
        ...
        sample_ens = qp.Ensemble(
            qp.hist, data=dict(bins=self.zgrid, pdfs=np.atleast_2d(hist_samples))
        )
        qp_d = qp.Ensemble(
            qp.hist, data=dict(bins=self.zgrid, pdfs=np.atleast_2d(hist_single))
        )
